...
<?php
$query="SELECT * FROM `drclark` ORDER BY `product` ASC ";
...
<td ><a  title="add to stock" onclick="return confirm('Add more to stock?')" href="<?php echo "add.php?id={$row['pid']}&table=drclark";?>">ADD</a></td>
	
<td><a  title="remove from stock" onclick="return confirm('remove from stock?')" href="<?php echo "remove.php?id={$row['pid']}&table=drclark";?>">REMOVE</a></td>

<td><a  title="view"  href="<?php echo "statement.php?id={$row['pid']}&table=drclark&name={$row['product']}";?>">INVENTORY STATEMENT</a></td>
