...
<?php
$query="SELECT * FROM `general` ORDER BY `product` ASC ";
...
<td ><a  title="add to stock" onclick="return confirm('Add more to stock?')" href="<?php echo "add.php?id={$row['pid']}&table=general";?>">ADD</a></td>
	
<td><a  title="remove from stock" onclick="return confirm('remove from stock?')" href="<?php echo "remove.php?id={$row['pid']}&table=general";?>">REMOVE</a></td>

<td><a  title="view"  href="<?php echo "statement.php?id={$row['pid']}&table=general&name={$row['product']}";?>">INVENTORY STATEMENT</a></td>
